...
actor RecognitionSession {
    ...
    private func currentASREndpoint() -> String {
        ...
        case .openai:
            guard let config = KeychainService.loadASRConfig(for: .openai) as? OpenAIASRConfig,
                  let baseURL = URL(string: config.baseURL)
            else { return "" }
            return baseURL.deletingLastPathComponent().appendingPathComponent("health").absoluteString
        default:
            return ""
        }
    }
    ...
    /// Stable ID used by streaming recovery checkpoints and the final history
    /// row. A crash leaves the latest partial under this ID; clean completion
    ...
    private func completeMacAction(
        ...
        let recordId = activeHistoryRecordID ?? UUID().uuidString
        let recordCreatedAt = activeHistoryCreatedAt ?? Date()
    ...
    func stopRecording() async {
        ...
        // LocalVoice/Qwen3 can only produce its first partial after one full
        // decoder chunk. Always send finish for it so sub-4-second utterances
        // are decoded instead of being mistaken for silence.
        if providerIsStreaming && provider != .openai {
            let duration = recordingStartTime.map { Date().timeIntervalSince($0) } ?? 0
    ...
    private func handleASREvent(_ event: RecognitionEvent, expectedGeneration: Int) async {
        ...
        // Persist transcript progress before SwiftUI sees it. If rendering the
        // floating panel ever crashes again, the text that triggered that
        // render is already durable in History rather than only in memory.
        if case .transcript(let transcript) = event {
            currentTranscript = transcript
            await checkpointHistoryIfNeeded(transcript.displayText)
        }
    ...
    private func checkpointHistoryIfNeeded(_ text: String) async {
        let text = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty,
              text != lastHistoryCheckpointText,
              let recordID = activeHistoryRecordID,
              let createdAt = activeHistoryCreatedAt
        else { return }

        lastHistoryCheckpointText = text
        let duration = recordingStartTime.map { Date().timeIntervalSince($0) } ?? 0
        await historyStore.upsertInterrupted(HistoryRecord(
            id: recordID,
            createdAt: createdAt,
            durationSeconds: duration,
            rawText: text,
    ...
    private func discardActiveHistoryCheckpoint() async {
        if let recordID = activeHistoryRecordID {
            await historyStore.delete(id: recordID)
        }
        clearActiveHistoryCheckpoint()
    }
    ...
    private func forceReset() async {
        ...
        state = .idle
        currentTranscript = .empty
        // Keep an already-persisted interrupted row, but detach it from the
        // next session so future checkpoints cannot overwrite it.
        clearActiveHistoryCheckpoint()
        hasEmittedReadyForCurrentSession = false
        currentConfig = nil
        uploadFailureFlag = nil
