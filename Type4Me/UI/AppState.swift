...
final class AppState {
    ...
    var recordingDurationAtStop: TimeInterval?
    ...
    var showsAudioBacklog: Bool {
        guard KeychainService.selectedASRProvider == .openai,
              let config = KeychainService.loadASRConfig(for: .openai) as? OpenAIASRConfig
        else { return false }
        return config.model.lowercased().contains("qwen3-asr")
    }
    ...
    func stopRecording() {
        ...
        case .recording:
            if let recordingStartDate {
                recordingDurationAtStop = max(0, Date().timeIntervalSince(recordingStartDate))
            }
            audioLevel.current = 0
    ...
    /// A repeated shortcut during finalization is intentionally not allowed to
    /// start a second capture, but it must never feel like a dead key.
    func showFinishingHint() {
        guard barPhase == .processing else { return }
        processingLabelOverride = L("上一段收尾中", "Finishing previous")
        onShowPanel?()
    }
    ...
    func setLiveTranscript(_ transcript: RecognitionTranscript) {
        if let processed = transcript.processedAudioSeconds {
            processedAudioSeconds = max(processedAudioSeconds, processed)
        }
        ...
        if transcript.processedAudioSeconds == nil,
           latencyMs > Self.stalePartialTranscriptThresholdMs,
           !transcript.isFinal,
           !segments.isEmpty {
