...
if [ "$ARCH" = "arm64" ]; then
    ...
    swift build -c release --package-path "$PROJECT_DIR" --arch arm64 2>&1 | grep -E "Build complete|Build succeeded|error:|warning:"
else
    echo "Building universal release (arm64 + x86_64)..."
    swift build -c release --package-path "$PROJECT_DIR" --arch arm64 --arch x86_64 2>&1 | grep -E "Build complete|Build succeeded|error:|warning:"
fi
...
if [ "$NEEDS_SIGN" = "1" ]; then
    ...
    if [ "$SIGNING_IDENTITY" = "-" ]; then
        ...
        # Without this, macOS defaults to a CDHash requirement, invalidating
        # Accessibility permission after every local rebuild.
        CODESIGN_ARGS+=(--requirements "=designated => identifier \"$APP_BUNDLE_ID\"")
    fi
    if [ -f "$ENTITLEMENTS" ]; then
